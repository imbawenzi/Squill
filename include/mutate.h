...
static char* ir_from_to_String[] = {
  "initSeeds", "initLib", "seeds"
};

static char* ir_favor_to_String[] = {
  "favored", "all"
};

static char* SQLSTATUS_enum_to_String[] = {
  "kConnectFailed", "kExecuteError", "kServerCrash", "kNormal", "kTimeout1", "kTimeout2", "kSyntaxError", "kSemanticError"
};
...
public:
    ...
    vector<IR *> havoc_mutate(vector<IR *> &v_ir_collector, int energy, vector<int> &v_havoc_count);
    ...
    void add_ir_to_library(IR*, vector<vector<int>> &ir_library_index);//DONE
    void add_ir_to_library_no_deepcopy(IR*, vector<vector<int>> &ir_library_index);
    ...
    void collect_ir(IR* root , vector<IR *> &v_ir_collector);
    ...
    //add
    vector<string> g_mutate_type;

    vector<string> g_mutate_method;
    vector<string> g_mutate_cur_method;
    
    vector<string> g_mutate_ir_from;
    vector<string> g_mutate_cur_ir_from;

    vector<string> g_mutate_ir_status;
    vector<string> g_mutate_cur_ir_status;

    vector<string> g_mutate_ir_favor;
    vector<string> g_mutate_cur_ir_favor;

    set<IRTYPE> delete_insert_mutatable_types_;

    // 0->init seeds, 1->init lib, 2->seeds
    int g_ir_from;
    int g_ir_status;
