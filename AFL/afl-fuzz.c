...
#define INIT_LIB_PATH "/home/ecs-user/testOB/MySQL/docker/init_lib"
#define SAFE_GENERATE_PATH "/home/ecs-user/testOB/MySQL/docker/safe_generate_type_mysql"
#define GLOBAL_TYPE_PATH "/home/ecs-user/testOB/MySQL/docker/global_data_lib_mysql"

#define RUN_PATH "/home/ecs-user/testOB/workdir/run_ob.sh"
#define PREPARE_PATH "/home/ecs-user/testOB/workdir/prepare.sh"
#define SHM_PATH "/home/ecs-user/testOB/workdir/shm.info"
...
class MysqlClient{
  public:
    ...
    int connect(){
      ...
      if(mysql_real_connect(&m_, host_, user_name_, passwd_, dbname.c_str(), 2881, NULL, CLIENT_MULTI_STATEMENTS) == NULL){
        int error_num = mysql_errno(&m_);
        writeLog("afl_fuzz.log", mysql_error(&m_));
        writeLog("afl_fuzz.log", to_string(error_num).c_str());
    ...
    SQLSTATUS execute(char * cmd){
      ...
      u64 execute_start, execute_stop;

      // string log_interesting = "g_if_interest: " + to_string(g_if_interest);
      // writeLog("afl_fuzz.log", log_interesting.c_str());
      ...
      int server_response = mysql_query(&m_, cmd);
      auto correctness = clean_up_connection(m_);

      if(correctness == kServerCrash){
        return kServerCrash;
      }

      if(server_response == 2013 || server_response == 2006){
        return kServerCrash;
      }
      ...
      if(check_res == false){
        writeLog("afl_fuzz.log", "check_server_alive works");
        return kServerCrash;
      }
    ...
    bool check_server_alive(){
      auto conn = connect();
      // writeLog("afl_fuzz.log", "connect:");
      // writeLog("afl_fuzz.log", to_string(conn).c_str());
      if(conn == 2013 || conn == 2006){
        return false;
      }
      disconnect();
      return true;
    ...
    void init_database(){
      ...
      string reset_query = "CREATE TABLE v0 ( v1 INT , v2 INT , v3 INT) ;";
      reset_query += "INSERT INTO v0 VALUES (1, 2, 3);";
      reset_query += "INSERT INTO v0 VALUES (3, 2, 1);";

      reset_query += "CREATE TABLE v4 ( v5 INT , v6 INT , v7 INT) ;";
      reset_query += "INSERT INTO v4 VALUES (4, 5, 6);";
      reset_query += "INSERT INTO v4 VALUES (6, 5, 4);";

      auto tmp_res = mysql_real_query(&m_, reset_query.c_str(), reset_query.size()); 
      int res = -1;
      do{
        auto q_result = mysql_store_result(&m_);
        if(q_result)
          mysql_free_result(q_result);
      }while((res = mysql_next_result(&m_))==0);
...
vector<int> g_havoc_count;

int g_all_type_num = sizeof(ALLTYPE_enum_to_String) / sizeof(ALLTYPE_enum_to_String[0]);
string g_cur_type;
string g_cur_method;

string g_cur_ir_from;
string g_cur_ir_status;
string g_cur_ir_favor;

// 跨文件的全局变量
vector<vector<int>> g_favored_ir_library_index;

vector<vector<int>> g_init_ir_library_index;


vector<vector<int>> g_ir_library_from;
vector<vector<int>> g_ir_library_status;

unsigned long g_id_counter = 0;

SQLSTATUS g_cur_status;

int g_kNormal_count = 0;
int g_kSemanticError_count = 0;
int g_kSyntaxError_count = 0;
...
struct queue_entry {
  ...
  vector<vector<int>> *ir_library_index;

  SQLSTATUS query_status;
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  q->ir_library_index = new vector<vector<int>>;
  q->ir_library_index->resize(g_all_type_num);
  ...
  for (i = 0; i < MAP_SIZE; i++)
    ...
    if (trace_bits[i]) {
       ...
       if (top_rated[i]) {
         ...
         if ((top_rated[i]->query_status != kNormal) && (q->query_status == kNormal)){
           goto replace_top;
         }

         if ((top_rated[i]->query_status != kSyntaxError) && (q->query_status == kSemanticError)){
           goto replace_top;
         }         
         ...
         if (q->query_status != kNormal) continue;
...
static void cull_queue(void) {
  ...
  //清零
  for (int i = 0;i < g_all_type_num;i++){
    g_favored_ir_library_index[i].clear();
    vector<int>().swap(g_favored_ir_library_index[i]);
  }

  g_kNormal_count = 0;
  g_kSemanticError_count = 0;
  g_kSyntaxError_count = 0;
  ...
  for (i = 0; i < MAP_SIZE; i++)
    if (top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7)))) {
      ...
      if (top_rated[i]->query_status == kNormal){
        g_kNormal_count += 1;
      }
      if (top_rated[i]->query_status == kSemanticError){
        g_kSemanticError_count += 1;
      }
      if (top_rated[i]->query_status == kSyntaxError){
        g_kSyntaxError_count += 1;
      }
      ...
      // 把favored的种子的IR加到g_favored_ir_library_index里

      if (top_rated[i]->query_status != kNormal){
        for (int cur = 0; cur < g_all_type_num; cur++){
          g_favored_ir_library_index[cur].insert(g_favored_ir_library_index[cur].end(), top_rated[i]->ir_library_index->at(cur).begin(), top_rated[i]->ir_library_index->at(cur).end());
        }
      }
...
EXP_ST void setup_shm(void) {
  ...
  string rm_shm = string("rm ") + SHM_PATH;
  system(rm_shm.c_str());
  writeLog(SHM_PATH, shm_str);
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    vector<IR*> ir_set;
    IR* root_ir;
    string input((const char*)g_current_input);
    Program* program = parser(input);
    if(program == NULL) {
      goto nothing_happend;
    }
    try{
      root_ir = program->translate(ir_set);
    }catch(...){
      program->deep_delete();
      goto nothing_happend;
    }
    ...
    g_mutator.g_ir_from = 0;
    g_mutator.g_ir_status = q->query_status;

    g_mutator.add_ir_to_library(root_ir, (* q->ir_library_index));
    deep_delete(root_ir);
  nothing_happend:
...
static u8* describe_op(u8 hnb) {
  ...
  sprintf(ret + strlen(ret), ",type:%s", g_cur_type.c_str());

  sprintf(ret + strlen(ret), ",method:%s", g_cur_method.c_str());

  sprintf(ret + strlen(ret), ",status:%s", SQLSTATUS_enum_to_String[g_cur_status]);

  sprintf(ret + strlen(ret), ",IRfrom:%s", g_cur_ir_from.c_str());

  sprintf(ret + strlen(ret), ",IRstatus:%s", g_cur_ir_status.c_str());

  sprintf(ret + strlen(ret), ",IRfavor:%s", g_cur_ir_favor.c_str());
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  // writeLog("afl_fuzz.log", "g_if_interest = 1");
  g_if_interest = 1;
  ...
  // writeLog("afl_fuzz.log", ("fault" + to_string(fault)).c_str());
  ...
  if (fault == crash_mode) {
    ...
    if (!(hnb = has_new_bits(virgin_bits))) {
      // writeLog("afl_fuzz.log", "g_if_interest = 0");
      g_if_interest = 0;
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fprintf(plot_file, 
          ...
          get_cur_time() / 1000, queue_cycle - 1, current_entry, queued_paths,
          pending_not_fuzzed, pending_favored, bitmap_cvg, unique_crashes,
          unique_hangs, max_depth, eps, total_execs, syntax_err_num, semantic_err_num, correct_num, g_kNormal_count, g_kSemanticError_count, g_kSyntaxError_count);
  #else
  fprintf(plot_file, 
...
#define SP20    SP10 SP10
  ...
  SAYF(bV bSTOP " total vaild : " cRST "%-21s " bSTG bV bSTOP 
  ...
  SAYF(bV bSTOP "     kNormal : " cRST "%-37d " bSTG bV bSTOP "    levels : "
       cRST "%-10s " bSTG bV "\n", g_kNormal_count, DI(max_depth));
  ...
  SAYF(bV bSTOP "kSemanticError:" cRST "%-37d " bSTG bV bSTOP "   pending : "
       cRST "%-10s " bSTG bV "\n", g_kSemanticError_count, DI(pending_not_fuzzed));
  ...
  SAYF(bV bSTOP "kSyntaxError : " cRST "%-37d " bSTG bV bSTOP "  pend fav : "
       cRST "%-10s " bSTG bV "\n", g_kSyntaxError_count, DI(pending_favored));
...
EXP_ST u8 common_fuzz_stuff(char** argv, const u8* out_buf, u32 len) {
  ...
  if(should_keep == 1){
    g_mutator.g_ir_from = 2;
    g_mutator.g_ir_status = g_cur_status;
    

    g_mutator.add_ir_to_library(g_current_ir, (* queue_top->ir_library_index));
  }
...
static u8 fuzz_one(char** argv) {
  ...
  cur_energy = HAVOC_CYCLES * perf_score / havoc_div / 100;
  ...
  // g_havoc_count.clear();
  // mutated_tree = g_mutator.havoc_mutate(ir_set, cur_energy, g_havoc_count);
  ...
  for(auto &ir: mutated_tree){
    ...
    g_cur_type = g_mutator.g_mutate_type[tmp_count];
    g_cur_method = g_mutator.g_mutate_method[tmp_count];

    g_cur_ir_from = g_mutator.g_mutate_ir_from[tmp_count];
    g_cur_ir_status = g_mutator.g_mutate_ir_status[tmp_count];
    g_cur_ir_favor = g_mutator.g_mutate_ir_favor[tmp_count];

    tmp_count++;

    stage_name = "niubi_validate";

    // stage_cur_val = g_havoc_count[stage_cur];
    stage_cur_val = 0;

    // 过滤一下太长的输入
    int total_node_num = calc_node(ir);
    if (total_node_num > 1500){
      skip_count ++ ;
      continue;
    }
...
int main(int argc, char** argv) {
  ...
  g_favored_ir_library_index.resize(g_all_type_num);

  g_init_ir_library_index.resize(g_all_type_num);

  g_ir_library_from.resize(g_all_type_num);
  g_ir_library_status.resize(g_all_type_num);

  g_mutator.not_mutatable_types_.insert({kProgram, kStmtlist, kStmt, kCreateStmt, kDropStmt, kCreateTableStmt, kCreateIndexStmt, kCreateTriggerStmt, kCreateViewStmt, kDropIndexStmt, kDropTableStmt, kDropTriggerStmt, kDropViewStmt, kSelectStmt, kUpdateStmt, kInsertStmt, kAlterStmt});
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q:s:c:l:b:")) > 0)
