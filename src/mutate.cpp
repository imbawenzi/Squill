...
void Mutator::collect_ir(IR* root , vector<IR *> &v_ir_collector){
    v_ir_collector.push_back(root);
    if (root->left_){
        collect_ir(root->left_, v_ir_collector);
    }
    if (root->right_){
        collect_ir(root->right_, v_ir_collector);
    }
}

vector<IR *> Mutator::havoc_mutate(vector<IR *> &v_ir_collector, int energy, vector<int> &g_havoc_count){

    if (energy < HAVOC_MUTATE_MIN) energy = HAVOC_MUTATE_MIN;

    vector<IR *> res;
    IR * root = v_ir_collector[v_ir_collector.size() - 1];

    for (int stage_cur = 0; stage_cur < energy; stage_cur++) {

        vector<IR*> copy_ir_vector;
        IR* cur_root = deep_copy(root);

        int use_stacking = 1 << (get_rand_int(3));
        g_havoc_count.push_back(use_stacking);
        for (int i = 0; i < use_stacking; i++){
            
            copy_ir_vector.clear();
            collect_ir(cur_root, copy_ir_vector);

            IR* cur_mutate_ir;
            do{
                cur_mutate_ir = vector_rand_ele(copy_ir_vector);
            } while(not_mutatable_types_.find(cur_mutate_ir->type_) != not_mutatable_types_.end());

            // 应该不存在只有右子节点没左子节点的情况
            if (cur_mutate_ir->left_ == NULL && cur_mutate_ir->right_ == NULL){
                switch (get_rand_int(2)) {
                    case 0:{
                    // add left node
                        for(int k=0; k<4; k++){
                            IR* fetch_ir = get_ir_from_library(cur_mutate_ir->type_);
                            if(fetch_ir->left_ != NULL){
                                cur_mutate_ir->left_ = deep_copy(fetch_ir->left_);
                                break;
                            }
                        }
                        break;
                    ...
                    case 1:{
                        for(int k=0; k<4; k++){
                            IR* fetch_ir = get_ir_from_library(cur_mutate_ir->type_);
                            if(fetch_ir->left_ != NULL && fetch_ir->right_ != NULL){
                                cur_mutate_ir->left_ = deep_copy(fetch_ir->left_);
                                cur_mutate_ir->right_ = deep_copy(fetch_ir->right_);
                                break;
                            }
                        }
                        break;
            ...
            if (cur_mutate_ir->left_ != NULL && cur_mutate_ir->right_ == NULL){
                // 0-2
                cur_rand_num = get_rand_int(3);
            }
            if (cur_mutate_ir->left_ != NULL && cur_mutate_ir->right_ != NULL){
                // 2-6
                cur_rand_num = get_rand_int(5) + 2;
            }
            if (cur_mutate_ir->left_ != NULL && cur_mutate_ir->right_ == NULL){
                // writeLog("afl_fuzz.log", "what happend?");
                continue;
            }
            ...
            switch (cur_rand_num) {
                ...
                case 0:{
                    for(int k=0; k<4; k++){
                        IR* fetch_ir = get_ir_from_library(cur_mutate_ir->type_);
                        if(fetch_ir->left_ != NULL && fetch_ir->left_->type_ == cur_mutate_ir->left_->type_ && fetch_ir->right_ != NULL){
                            cur_mutate_ir->right_ = deep_copy(fetch_ir->right_);
                            break;
                        }
                    }
                ...
                // delete left node
                case 1:{
                    deep_delete(cur_mutate_ir->left_);
                    cur_mutate_ir->left_ = NULL;
                    break;
                }
                // replace left node
                case 2:{
                    IR* new_node = get_ir_from_library(cur_mutate_ir->left_->type_);
                    deep_delete(cur_mutate_ir->left_);
                    cur_mutate_ir->left_ = deep_copy(new_node);
                    break;
                }
                // delete right node
                case 3:{
                    deep_delete(cur_mutate_ir->right_);
                    cur_mutate_ir->right_ = NULL;
                    break;
                }
                // replace right node
                case 4:{
                    IR* new_node = get_ir_from_library(cur_mutate_ir->right_->type_);
                    deep_delete(cur_mutate_ir->right_);
                    cur_mutate_ir->right_ = deep_copy(new_node);
                    break;
                }
                // delete both node
                case 5:{
                ...
                // replace both node
                case 6:{
                    IR* new_left_node = get_ir_from_library(cur_mutate_ir->left_->type_);
                    deep_delete(cur_mutate_ir->left_);
                    cur_mutate_ir->left_ = deep_copy(new_left_node);

                    IR* new_right_node = get_ir_from_library(cur_mutate_ir->right_->type_);
                    deep_delete(cur_mutate_ir->right_);
                    cur_mutate_ir->right_ = deep_copy(new_right_node);
                    break;
                }
            }    
        }
        // 去重
        extract_struct(cur_root);
        string tmp = cur_root->to_string();
        unsigned tmp_hash = hash(tmp);
        if(global_hash_.find(tmp_hash) != global_hash_.end()){
            deep_delete(cur_root);
            continue;
        }
...
vector<IR *> Mutator::mutate_all(vector<IR *> &v_ir_collector){
    ...
    g_mutate_type.clear();
    vector<string>().swap(g_mutate_type);

    g_mutate_method.clear();
    vector<string>().swap(g_mutate_method);

    g_mutate_ir_from.clear();
    vector<string>().swap(g_mutate_ir_from);

    g_mutate_ir_status.clear();
    vector<string>().swap(g_mutate_ir_status);  

    g_mutate_ir_favor.clear();
    vector<string>().swap(g_mutate_ir_favor);

    for(auto ir: v_ir_collector){
        if(not_mutatable_types_.find(ir->type_) != not_mutatable_types_.end()) continue;

        g_mutate_cur_method.clear();
        vector<string>().swap(g_mutate_cur_method);

        g_mutate_cur_ir_from.clear();
        vector<string>().swap(g_mutate_cur_ir_from);

        g_mutate_cur_ir_status.clear();
        vector<string>().swap(g_mutate_cur_ir_status);

        g_mutate_cur_ir_favor.clear();
        vector<string>().swap(g_mutate_cur_ir_favor);
        ...
        for(auto i: v_mutated_ir){
            ...
            string cur_method = g_mutate_cur_method[tmp_count];
            string cur_from = g_mutate_cur_ir_from[tmp_count];
            string cur_status = g_mutate_cur_ir_status[tmp_count];
            string cur_favor = g_mutate_cur_ir_favor[tmp_count];
            ...
            string cur_type = ALLTYPE_enum_to_String[i->type_];
            
            g_mutate_type.push_back(cur_type);
            g_mutate_method.push_back(cur_method);

            g_mutate_ir_from.push_back(cur_from);
            g_mutate_ir_status.push_back(cur_status);
            g_mutate_ir_favor.push_back(cur_favor);
...
void Mutator::add_ir_to_library(IR * cur, vector<vector<int>> &ir_library_index){

    extract_struct(cur);
    cur = deep_copy(cur);
    add_ir_to_library_no_deepcopy(cur, ir_library_index);
    return;
}

void Mutator::add_ir_to_library_no_deepcopy(IR * cur, vector<vector<int>> &ir_library_index){
    if(cur->left_) add_ir_to_library_no_deepcopy(cur->left_, ir_library_index);
    if(cur->right_) add_ir_to_library_no_deepcopy(cur->right_, ir_library_index);
    ...
    //获取这个插入ir在ir_library_里的index
    int cur_index = ir_library_[type].size() - 1;

    ir_library_index[type].push_back(cur_index);

    g_ir_library_from[type].push_back(g_ir_from);
    g_ir_library_status[type].push_back(g_ir_status);
...
void Mutator::init_ir_library(string filename){
    ...
    while(getline(input_file, line)){
        ...
        g_ir_from = 1;
        g_ir_status = 999;
        add_ir_to_library(res, g_init_ir_library_index);
...
// #define MYSQLFUZZ
    ...
    delete_insert_mutatable_types_.insert({kOptWithClause, kOptOrderClause, kOptLimitClause, kOptAllOrDistinct, kOptFromClause, kOptWhereClause, kOptGroupClause, kOptWindowClause, kOptExistWindowName, kOptPartition, kOptFrameClause, kOptHavingClause, kOptTablePrefix, kOptAsAlias, kOptIndex, kOptOn, kOptUsing, kOptJoinType, kOptLimitRowCount, kOptOrderBehavior, kOptColumnNameListP, kOptTemp, kOptIfNotExist, kOptTableOptionList, kOptCreateDefinitionListP, kOptIgnoreOrReplace, kOptAs, kOptIndexType, kOptIndexKeyword, kOptIndexOption, kOptExtraOption, kOptViewAlgorithm, kOptSqlSecurity, kOptCheckOption, kOptOpComma, kOptOpEqual, kOptIfExist, kOptRestrictOrCascade, kOptPriority, kOptIgnore, kOptInto, kOptAsRowAlias, kOptOnDuplicate, kOptLowPriority, kOptColAlias, kOptColumn, kOptIndexName, kOptIndexOrKey, kOptConstraintSymbol, kOptNullNotNull, kOptVisibleInvisible, kOptAutoIncrement, kOptUniquePrimary, kOptColumnFormat, kOptStorage, kOptReferenceDefinition, kOptCheckConstraintDefinition, kOptMatch, kOptOnDelete, kOptOnUpdate, kOptEnforced, kOptNot, kOptOverClause, kOptDistinct, kOptLength});
...
vector<IR *> Mutator::mutate(IR * input){
    ...
    if(tmp != NULL){
        ...
        g_mutate_cur_method.push_back(method_delete); 

        string ir_from = "none";
        g_mutate_cur_ir_from.push_back(ir_from);

        string ir_status = "none";
        g_mutate_cur_ir_status.push_back(ir_status);

        string ir_favor = "none";
        g_mutate_cur_ir_favor.push_back(ir_favor);

        res.push_back(tmp);
    }
    if(delete_insert_mutatable_types_.find(input->type_) != delete_insert_mutatable_types_.end()){
        tmp = strategy_insert(input);
        if(tmp != NULL){
            string method_insert = "insert";
            g_mutate_cur_method.push_back(method_insert);     

            string ir_from = ir_from_to_String[g_cur_ir_from];
            g_mutate_cur_ir_from.push_back(ir_from);

            string ir_status = SQLSTATUS_enum_to_String[g_cur_ir_status];
            g_mutate_cur_ir_status.push_back(ir_status);

            string ir_favor = ir_favor_to_String[g_cur_ir_favor];
            g_mutate_cur_ir_favor.push_back(ir_favor);
    ...
    if(tmp != NULL){
        ...
        g_mutate_cur_method.push_back(method_replace);   
...
IR * Mutator::strategy_delete(IR * cur){
    ...
    DOLEFT
        res = deep_copy(cur);
        if((res->left_ != NULL) && (delete_insert_mutatable_types_.find(res->left_->type_) != delete_insert_mutatable_types_.end())){
    ...
    DORIGHT
        res = deep_copy(cur);
        if((res->right_ != NULL) && (delete_insert_mutatable_types_.find(res->right_->type_) != delete_insert_mutatable_types_.end())){
...
//     int ran_index = get_rand_int(ir_library_[type].size());

//     g_cur_ir_from = g_ir_library_from[type][ran_index];
//     g_cur_ir_status = g_ir_library_status[type][ran_index];

//     return ir_library_[type][ran_index];
// }
...
IR* Mutator::get_ir_from_library(IRTYPE type){
    ...
    if(ir_library_[type].empty()){
        // writeLog("afl_fuzz.log", "ir_library_ empty");
        return empty_ir;
    }
    ...
    if (rand_int < 5){
        ...
        if (g_favored_ir_library_index[type].size() == 0){
            goto all;
        }
        int cur_ir_library_index = vector_rand_ele(g_favored_ir_library_index[type]);

        g_cur_ir_from = g_ir_library_from[type][cur_ir_library_index];
        g_cur_ir_status = g_ir_library_status[type][cur_ir_library_index];

        // if (g_cur_ir_status == 3){
        //     string output = to_string(g_cur_ir_from);
        //     writeLog("afl_fuzz.log", output.c_str());
        // }

        return ir_library_[type][cur_ir_library_index];
    }
    else{
    all:
...
void Mutator::extract_struct(IR * root){
    ...
    if(root->str_val_.size() != 0){
        root->str_val_ = "x"; 
        return ;     
    }

    if(type == kStringLiteral){
        root->str_val_ = "'x'";
    }else if(type == kIntLiteral){
        root->int_val_ = 1;
    }else if(type == kFloatLiteral){
        root->float_val_ = 1.0;
    }
}
...
bool Mutator::fix(IR * root){
    map<IR**, IR*> m_save;
    bool res = true;
    // mutate.h     119:    set<IRTYPE> split_stmt_types_;
    // mutate.cpp   228:    split_stmt_types_.insert(kStmt);
    // vector<IR *> stmts ??????kStmt???IR
    // ?????????????????
    auto stmts = split_to_stmt(root, m_save, split_stmt_types_);

    if(stmts.size() > 8) {
        connect_back(m_save); 
        cout << "stmts.size() > 8" << endl;
        return false;
    }
    ...
    for(auto &stmt: stmts){
        ...
        // ??????
        // {kStmt, kSelectClause, kSelectStmt}
        // ????????Select??????????
        // Stmt->SelectStmt->SelectNoParens->SelectClauseList->SelectClause
        // ???????????substmts????????stmt
        ...
        if(stmt_num > 4) {
            ...
            cout << "stmt_num > 4" << endl;
            cout << "stmt_num now is" << stmt_num << endl;
            return false;
        }
        ...
        for(auto &substmt: substmts){
            ...
            if((stmt_num == 1 && tmp_node_num > 150) || tmp_node_num > 120) {
                connect_back(m_save);
                connect_back(m_substmt_save);
                cout << "(stmt_num == 1 && tmp_node_num > 150) || tmp_node_num > 120)" << endl;
                return false;
            }
...
bool Mutator::connect_back(map<IR**, IR*> &m_save){
    // https://blog.csdn.net/weixin_42240667/article/details/103019910
...
map<IR*, vector<IR*>> Mutator::build_graph(IR * stmt_root, map<int, map<DATATYPE, vector<IR*>>> &scope_library){
    ...
    //cout << "stmt root" << stmt_root->to_string() << endl;

    while(!bfs.empty()){
        // BFS????
        auto node = bfs.front();

        //cout << "current node" << node->to_string() << endl;
        ...
        // ??????????{kDataColumnName, kDataPragmaValue, kDataTableName}??
        if(relationmap_.find(cur_data_type) != relationmap_.end()){     
            auto &target_data_type_map = relationmap_[cur_data_type];
            // ??kDataPragmaValue???target.first?kDataPragmaKey ?????????????
            // ??kDataColumnName???target.first???kDataTableName?kDataColumnName
            // ??kDataTableName???target.first?kDataTableName
            for(auto &target: target_data_type_map){
                IR* pick_node = NULL;
                // ??data_flag???9?????
                // ???????????
                // kMapToClosestOne???bison.y????
                // ???MYSQL?????if??????
                if(isMapToClosestOne(cur_data_flag)){
                    ...
                    // ??????????kDataColumnName?kDataTableName????
                ...
                if(pick_node != NULL)
                    //cout << "pick node" << node->to_string() << endl;
...
bool Mutator::fill_one(IR* ir){
    ...
    // ?????????DATAFLAG?????
    // kDefine?????????????
    // ??Create Table??????TableName??DATAFLAG??kDefine
    if(isDefine(ir->data_flag_)){
        ...
        return true;
    // kAlias?????????????????????
    // CREATE TABLE AS?CREATE INDEX????DATAFLAG
